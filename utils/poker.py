...
class Judge(object):
    ...
    def __init__(self) -> None:
        ...
        '''
        with open('/home/LuZQ/RL_poker/dic_point.dic', 'rb') as f:
...
class Games_info(TypedDict):
    ...
    public_cards  : list
    all_in_player : list
    card_point    : name_dict



class Game(object):
    
    def __init__(self, games_info:Games_info, judge, logging_level = logging.WARNING, log_filename=None):
        '''
        games_info : dict
            keys:
                names         : list
    ...
    def actions(self, player, action):
        ...
        if action < need_to_call:
            action = 0
        elif action < need_to_call + self.pot * 0.2:
            action = 1
